...
grafo da exportação (ver ``infrastructure/ffmpeg/composer.py``), e aqui só se toca o PCM que sai
...
class AudioPreview(QObject):
    ...
    def available(self) -> bool:
        ...
        if not self._enabled or not MULTIMEDIA_AVAILABLE:
            return False
        return not QMediaDevices.defaultAudioOutput().isNull()

    @property
    def playing(self) -> bool:
        # O EOF do produtor pode chegar com áudio ainda no buffer da placa.
        # O relógio continua válido até stop(), evitando cortar esse final.
    ...
    def start(self, command: list[str], at: float, *, pcm_format: tuple[int, int] | None = None) -> bool:
        """Começa a tocar a mixagem produzida por ``command``, situada em ``at``."""
        self.stop()
        if not self.available:
            return False

        rate, channels = pcm_format or (SAMPLE_RATE, CHANNELS)
        fmt = QAudioFormat()
        ...
        if not device.isFormatSupported(fmt):
            # O compositor entrega s16le. Interpretar esses bytes como o
            # formato preferido da placa (por exemplo float) distorce o som.
            return False
    ...
    def _read(
        ...
        try:
            ...
            # O último pedaço pode ocupar a última vaga. Perder o EOF nesse
            # instante deixava o player esperando áudio que nunca chegaria.
            while True:
                try:
